...
use crate::usj_generator::serde_json::Value;
...
pub fn node_2_usj_char(
    ...
    // Get tag node and collect children
    let tag_node = node.children(&mut node.walk()).next().unwrap();
    let children: Vec<_> = node.children(&mut node.walk()).collect();
    let mut children_range = children.len();
    if node.children(&mut node.walk()).last().unwrap().kind().starts_with('\\') {
        children_range -= 1;
    }
    ...
    for child in &children[1..children_range] {
        if child.kind().ends_with("Attribute") {
            let attrib_name_node = child.child(0).expect("Node should have at least one child");
            ...
            // Get attribute value
            let query_source = "((attributeValue) @attrib-val)";
            let query = Query::new(&tree_sitter_usfm3::language(), query_source)
                .expect("Failed to create query");
            let mut cursor = QueryCursor::new();
            let mut captures = cursor.matches(&query, *child, usfm.as_bytes());
            
            let attrib_value = if let Some(capture) = captures.next() {
                if let Some(value_capture) = capture.captures.get(0) {
                    value_capture.node.utf8_text(usfm.as_bytes())
                        .unwrap_or("")
                        .trim()
                        .to_string()
                } else {
            ...
            if attrib_name.starts_with("x-") {
                // Keep custom attributes as is
                char_json_obj[&attrib_name] = json!(attrib_value);
            } else if attrib_name == "|" {
                // Handle default attributes
                let attr_name = if style == "jmp" {
                    "link-id".to_string()
                } else {
                    DEFAULT_ATTRIB_MAP
                        .iter()
                        .find(|&&(key, _)| key == node.kind())
                        .map(|&(_, value)| value.to_string())
                        .unwrap_or(attrib_name)
                };
                char_json_obj[attr_name] = json!(attrib_value);
            } else if attrib_name == "src" {
                char_json_obj["file"] = json!(attrib_value);
            }

        } else if child.kind() == "text" {
            let text = child.utf8_text(usfm.as_bytes())
                .unwrap_or("")
                .trim();
            if !text.is_empty() {
                char_json_obj["content"].as_array_mut().unwrap().push(json!(text));
            }
        } else {
            node_2_usj(child, char_json_obj["content"].as_array_mut().unwrap(), usfm);
        }
    }

    // Only keep content array if it has content or is a jmp marker
    if char_json_obj["content"].as_array().unwrap().is_empty() && style != "jmp" {
        char_json_obj.as_object_mut().unwrap().remove("content");
    }

    parent_json_obj.push(char_json_obj);
}
...
pub fn node_2_usj_attrib(
    ...
    if attrib_name == "|" {
        if let Some(parent_node) = node.parent() {
            ...
            if parent_type == "jmp" {
                attrib_name = "link-id".to_string();
            } else {
                attrib_name = DEFAULT_ATTRIB_MAP
                    .iter()
                    .find(|&&(key, _)| key == parent_type)
                    .map(|&(_, value)| value.to_string())
                    .unwrap_or(attrib_name);
            }
        }
    ...
    // Add attribute to the last item in parent array
    if let Some(last_obj) = parent_json_obj.last_mut() {
        if let Some(obj) = last_obj.as_object_mut() {
            obj.insert(attrib_name, json!(attrib_value));
        }
    }
}
...
pub fn node_2_usj_table(
    ...
    if node.kind() == "table" {
        ...
        for child in node.children(&mut node.walk()) {
            node_2_usj(&child, table_json_obj["content"].as_array_mut().unwrap(), usfm);
        }

        parent_json_obj.push(table_json_obj);
    } else if node.kind() == "tr" {
        ...
        // Process children starting from index 1 (skip first child)
        let children: Vec<_> = node.children(&mut node.walk()).collect();
        for child in &children[1..] {
            node_2_usj(child, row_json_obj["content"].as_array_mut().unwrap(), usfm);
        }

        parent_json_obj.push(row_json_obj);
    } else if TABLE_CELL_MARKERS.contains(&node.kind()) {
        // Get style from first child node
        let tag_node = node.children(&mut node.walk()).next()
            .expect("Expected a child node for tag");
        let style = tag_node
            .utf8_text(usfm.as_bytes())
        ...
        let children: Vec<_> = node.children(&mut node.walk()).collect();
        for child in &children[1..] {
            node_2_usj(child, cell_json_obj["content"].as_array_mut().unwrap(), usfm);
        }
...
pub fn node_2_usj_special(
    ...
    if node.kind() == "esb" {
        ...
        for child in &children[1..children.len()-1] {
            node_2_usj(child, sidebar_json_obj["content"].as_array_mut().unwrap(), usfm);
        }

        parent_json_obj.push(sidebar_json_obj);
    } else if node.kind() == "cat" {
        let query_source = "((category) @category)";
        let query = Query::new(&tree_sitter_usfm3::language(), query_source)
            .expect("Failed to create query");
        let mut query_cursor = QueryCursor::new();

        if let Some(capture) = query_cursor.matches(&query, *node, usfm.as_bytes()).next() {
            if let Some(category_capture) = capture.captures.get(0) {
                let category = category_capture.node
                    .utf8_text(usfm.as_bytes())
                    .expect("Failed to get category text")
                    .trim()
                    .to_string();

                if let Some(last) = parent_json_obj.last_mut() {
                    last["category"] = json!(category);
                }
            }
        }
    } else if node.kind() == "fig" {
        ...
        let children: Vec<_> = node.children(&mut node.walk()).collect();
        for child in &children[1..children.len()-1] {
            node_2_usj(child, fig_json_obj["content"].as_array_mut().unwrap(), usfm);
        }
...
"#;
    ...
    // Create temporary content array for attributes
    let mut temp_content = Vec::new();

    // Process all children for attributes and content
    let children: Vec<_> = node.children(&mut node.walk()).collect();
    for child in &children[1..children.len()-1] {
        if child.kind().ends_with("Attribute") {
            node_2_usj_attrib(child, &mut temp_content, usfm);
        } else if child.kind() != "text" || !child.utf8_text(usfm.as_bytes()).unwrap().trim().is_empty() {
            if !ms_json_obj.as_object().unwrap().contains_key("content") {
                ms_json_obj["content"] = json!([]);
            }
            node_2_usj(child, ms_json_obj["content"].as_array_mut().unwrap(), usfm);
        }
    }

    // Transfer any attributes from temp_content to ms_json_obj
    if !temp_content.is_empty() {
        if let Some(attr_obj) = temp_content.first() {
            if let Some(attr_map) = attr_obj.as_object() {
                for (key, value) in attr_map {
                    ms_json_obj[key] = value.clone();
                }
            }
        }
    }
...
pub fn node_2_usj_generic(
    ...
    // Get tag node and style
    let tag_node = node.child(0).expect("Expected a child node for tag");
    let mut style = if tag_node.kind().starts_with('\\') {
        tag_node
            .utf8_text(usfm.as_bytes())
            .expect("Failed to get tag node text")
            .replace("\\", "")
    ...
    // Check if the second child is a numbered style
    if node.named_child_count() > 1 {
        if let Some(num_node) = node.named_child(1) {
            if num_node.kind().starts_with("numbered") {
                let num = num_node
                    .utf8_text(usfm.as_bytes())
                    .expect("Failed to get number text");
                style.push_str(num);
    ...
    // Process children starting after the style node
    let mut cursor = node.walk();
